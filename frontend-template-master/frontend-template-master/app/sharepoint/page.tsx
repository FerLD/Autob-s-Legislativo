...
import { Upload, Download, FileText, Users, Globe, ChevronRight, MoreVertical, SortAsc, X, ExternalLink, Check, ChevronDown, Pencil, Save } from 'lucide-react';
...
export default function SharePointPage() {
    const [listaDocumentos, setListaDocumentos] = useState(documentosIniciales);
    const [selectedDoc, setSelectedDoc] = useState<typeof documentosIniciales[0] | null>(null);
    const [selectedDocs, setSelectedDocs] = useState<typeof documentosIniciales>([]);
    const [mostrarMenuOrden, setMostrarMenuOrden] = useState(false);
    const [tipoOrden, setTipoOrden] = useState<TipoOrden>('nombre-asc');

    // NUEVO: Estados para edición solo en panel
    ...
    ? `${(file.size / (1024 * 1024)).toFixed(2)} MB`
        ...
        setListaDocumentos([nuevoDocumento,...listaDocumentos]);
        setSelectedDoc(nuevoDocumento);
        event.target.value = '';
    };

    const descargarDocumento = (doc: typeof documentosIniciales[0]) => {
        if (!doc.url || doc.url === "#") {
    ...
    const toggleDocSelection = (doc: typeof documentosIniciales[0]) => {
    ...
    const ordenarDocumentos = (tipo: TipoOrden) => {
        const documentosOrdenados = [...listaDocumentos].sort((a, b) => {
            switch (tipo) {
                case 'nombre-asc':
        ...
        setListaDocumentos(documentosOrdenados);
        setTipoOrden(tipo);
        setMostrarMenuOrden(false);
    };

    // NUEVO: Funciones de edición en panel
    ...
    const guardarEdicion = () => {
        if (!selectedDoc ||!editandoCampo ||!valorTemp.trim()) return;

        const nuevosDocs = listaDocumentos.map(doc =>
            doc.nombre === selectedDoc.nombre && doc.autor === selectedDoc.autor
              ? {...doc, [editandoCampo]: valorTemp.trim() }
                : doc
        );

        setListaDocumentos(nuevosDocs);
...
return (
    ...
    {/* TITULO */}
        <div className="mb-6 relative">
            ...
            Repositorio del Centro de Estudios Parlamentarios
            </p>
        </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/80 dark:border-gray-800/80 overflow-hidden shadow-sm dark:shadow-black/10">
                ...
                {listaDocumentos.map((doc, i) => {
                    ...
                    return (
                        <div key={i} onClick={() => setSelectedDoc(doc)} className={`grid grid-cols-12 items-center p-4 border-b border-gray-100 dark:border-gray-800/40 last:border-0 transition-all duration-200 hover:bg-primary/5 dark:hover:bg-[#C5AB81]/5 cursor-pointer group ${selectedDoc?.nombre === doc.nombre? 'bg-primary/5 dark:bg-[#C5AB81]/10' : ''}`}>
        ...
        <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white dark:bg-gray-950 border-l border-gray-200 dark:border-gray-800/80 shadow-2xl z-50 transition-transform duration-300 transform flex flex-col ${selectedDoc? 'translate-x-0' : 'translate-x-full'}`}>
                    ...
                    {selectedDoc?.url && selectedDoc.url!== "#"? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center">
                            ...
                            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">
                                {selectedDoc? "Vista previa no disponible para este mock" : "Selecciona un archivo"}
                            </p>
                        </div>
                    )}
                </div>
            ...
            {/* ACCIONES DEL PANEL - COMO ESTABAN */}
